# NodeStory

节点化叙事生成器，用于从自然语言故事生成可视化的分支节点剧情。

## 功能

- 在页面输入 OpenAI API key，并通过 `gpt-4o` 验证。
- 读取 `NodeStructure/*.txt` 中的五种节点结构。
- 读取相对路径 `../../Doc/Level_AI` 下的素材：
  - `NPC.csv`
  - `Item.csv`
  - `Building.csv`
- 先用 `gpt-4o` 判断故事适合哪一种结构。
- 再用 `gpt-5.5` 按结构生成完整节点故事。
- 如果输入的是经典故事、历史桥段或文学母题，会先提炼核心精神，再用当前 NPC、道具和地点改编，而不是机械替换表层元素。
- 结构选择会根据五种结构的适用场景逐一评分，再选择最匹配故事运动方式的结构。
- 使用类似 Fabula / DND 发牌的叙事准备法，生成前提牌、冲突牌、地点牌、NPC 牌、道具牌、转折牌、代价牌和结局牌。
- 将同一层节点视为平级叙事选项，保证它们有接近的篇幅、风险、价值和戏剧重量。
- 故事保持直接清楚：一个主冲突、一个清晰目标，分支是解决同一问题的不同方式，而不是不断扩散的新支线。
- 默认控制核心 NPC 和核心道具数量，避免把素材表里的角色和物品都塞进同一个故事。
- 节点文本采用清楚的剧情场景风格，可以包含 NPC 说了什么、走向谁、做了什么、物品和地点如何参与剧情；也会写出玩家可以如何赞同、帮助、说服、给出证据或选择立场，但不把可选玩家行为写成已经发生的剧情。
- 关键前因后果会通过 NPC 台词、可见行动或可检查物品讲清楚，让玩家知道谁背叛了谁、谁欠了谁、为什么当前冲突重要。
- 完整故事底稿会先写成一个顺畅的故事，不直接列出分支，但会保留后续可拆成节点和分支的关键决策点。
- 输出结构图、节点卡片和可导出的 JSON。

## 运行

```powershell
npm start
```

打开：

```text
http://localhost:4173
```

也可以双击：

```text
启动NodeStory.bat
```

## 路径说明

当前项目路径：

```text
F:\ChronicleForge\LLM_AI\NodeStory
```

素材路径使用相对位置：

```text
../../Doc/Level_AI
```

这样项目迁移到云端时，只要保持 `LLM_AI/NodeStory` 与 `Doc/Level_AI` 的相对层级一致，就能继续读取素材。

## 输出内容

整体故事包含：

- 自动选择的结构
- 选择理由
- 五种结构的选择评分
- 改编原则
- 故事上下文，包括玩家身份、主冲突、事发前因、失败代价
- 叙事发牌结果
- 完整故事底稿，作为后续节点拆分的详细基础
- 每层节点的平级规则
- 节点结构图

每个节点包含：

- 层级
- 节点作用
- 节点开始状态
- 更详细的节点剧情，包含 NPC 动作、短对白、道具或地点参与
- 玩家需要知道的前因后果
- 玩家可推动方式
- 节点结果，并为下一节点的开始状态做铺垫
- 出现的 NPC
- 出现的道具
- 故事发生地点
- 完成节点并开启下一节点的条件
- 后续节点

每条边包含：

- 短标签
- 自然衔接说明，说明上一节点改变了什么，以及为什么下一节点变得可进入
